...
echo"<br>";
	...
	//جامعات اهلية
	if($user_role == 140){
		$where_role=" and universty_lookup.univ_role = 11";
	}
	//جامعات تكنولوجية
	if($user_role == 150){
		$where_role=" and universty_lookup.univ_role = 12";
	}
	
  $sql ="SELECT applicant.`Serial` as app_serial ,`applicant`.`univ_id` as u_i_a,`applicant`.`user_role` as u_role $selectt ,`degree`,`FK_matlob`,matlob.matlob_NAME,request.StatusID,request.REQUEST_ID,status.STATUS_VALUE,
  status.STATUS_ID,`request`.`destination`,`request`.`univ_destination`,`request`.`univ_emkanyat`,`request`.`univ_gadwa`,`request`.`scu_destination`,`request`.`mozakera_dest`,`request`.`estefaa_dest` ,
  `request`.`estefa2`,`request`.`accept_dest`,`request`.`keta3_estefaa_dest`,
`request`.`fk_session_id`,universty_lookup.id as idd,universty_lookup.name as namee
FROM applicant , request $fromm ,matlob,status,universty_lookup
where 
...
and `applicant`.`user_role`=10
and universty_lookup.id = applicant.univ_id
 $wheree $where_role
	...
	while($rowsession = $ressession->fetch_array(MYSQLI_ASSOC))
	  ...
	  // echo $id_session;
	   ...
	   if($u_role==10)//univ
	   {
		...
		$idd=$row['idd'];
			$namee=$row['namee'];
		   /*$qunivac="SELECT `id`,`name` FROM `universty_lookup` where `id`=? and univ_role=10";
