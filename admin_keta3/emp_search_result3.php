		...
		if($user_role==120)
{
	...
	if($txtarname=="-1")
	{
		$wheree="and keta3.keta3_ID = `request`.`fk_keta3`  ";
	}
	else
	{
...
echo"<br>";
  ...
  $sql ="SELECT applicant.`Serial` as app_serial,`applicant`.`user_role` as approle , `applicant`.`univ_id` as appuniv  $selectt ,`degree`,`FK_matlob`,matlob.matlob_NAME,request.StatusID,request.REQUEST_ID,status.STATUS_VALUE,`request`.`destination`,`request`.`univ_destination`,`request`.`univ_emkanyat`,`request`.`univ_gadwa`,`request`.`scu_destination`
