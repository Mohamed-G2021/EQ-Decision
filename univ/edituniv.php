...
//echo '--------------------------------------'.$n;
	...
	$q="
	SELECT applicant.`Serial`,applicant.`fk_degree`,applicant.`univ_id`,applicant.`user_role`,applicant.`USER_ID`,applicant.`degree`,applicant.`FK_matlob`,request.`destination`,request.`univ_destination`,
	request.`estefaa_dest`,request.`univ_emkanyat`,request.`univ_gadwa`,request.`REQUEST_ID`,request.`keta3_estefaa_dest` 
...
if($user_role==12)//technological university
...
<!------------------------------------------------------------------------------------------------------------------------------------------------->	 
      ...
      <tr>
		...
		$sql = 'SELECT distinct id,name FROM  degree where id=?';
		$stmt = $con->prepare($sql);
...
<!------------------------------------------------------------------------------------------------------------------------------------------------->
 ...
 oninvalid="this.setCustomValidity('يجب ادخال التخصص')"
...
?>
  <tr>
        ...
        <td>
		<div align="right"> اللائحة الداخليه
		</div>
		</td>
      </tr>
	  <tr>
...
{ 
  ...
  <tr>
	...
	<td class="text-center"><a  href="3ardfile.php?id=<?php echo $univ_destination; ?>" target="_blank" >عرض</a>  </td>
...
?>
  <tr>
        ...
        <td>
		<div align="right"> الأمكانيات البشريه
		</div>
		</td>
      </tr>
	  <tr>
	  <input type="hidden" name="univ_emkanyat" id="univ_emkanyat" value="<?php echo"" ?>"  />	
	  </tr>
<?php }
else
{ 
  ...
  <tr>
        ...
        <td>
		<div align="right">
         <input type="file" name="univ_emkanyat" id="univ_emkanyat" accept="application/pdf"   >
        </div>
		</td>
	
	<td class="text-center"><a  href="3ardfile.php?id=<?php echo $univ_emkanyat; ?>" target="_blank" >عرض</a>  </td>
	  ...
	  <tr>
	  <input type="hidden" name="univ_emkanyat" id="univ_emkanyat" value="<?php echo $univ_emkanyat; ?>"  />	
	  </tr>
<?php 
} ?>
...
?>
  <tr>
        ...
        <td>
		<div align="right"> اخرى (الجواب و القرار الجمهورى للجامعه - أو االجواب و القرار الوزارى للمعهد)
		</div>
		</td>
      </tr>
	  <tr>
	  <input type="hidden" name="univ_gadwa" id="univ_gadwa" value="<?php echo"" ?>"  />	
	  </tr>
<?php }
else
{ 
  ...
  <tr>
        ...
        <td>
		<div align="right">
         <input type="file" name="univ_gadwa" id="univ_gadwa" accept="application/pdf"   >
        </div>
		</td>
	
	<td class="text-center"><a  href="3ardfile.php?id=<?php echo $univ_gadwa; ?>" target="_blank" >عرض</a>  </td>
	  ...
	  <tr>
	  <input type="hidden" name="univ_gadwa" id="univ_gadwa" value="<?php echo $univ_gadwa; ?>"  />	
	  </tr>
<?php 
} ?>
...
<?php
//echo $StatusID;
if ($StatusID==1 || $StatusID==13) // طلب مقدم
{
...
<td class="text-center"> لم يتم رفع ملف الاستيفاء </td>

	<?php }else{ ?>
	<td class="text-center"><a  href="3ardfile.php?id=<?php echo $estefaa_dest; ?>" target="_blank" >عرض</a>  </td>
<?php } ?>
        <td>
		<div align="right"> ملف الاستيفاء 
		</div>
		</td>
      </tr>
	  <tr>
	  <input type="hidden" name="estefaa_dest" id="estefaa_dest" value="<?php echo $estefaa_dest; ?>"  />	
	  </tr>
	<tr>
        <td dir="rtl"><div align="right">
          <input type="text" name="txtes" id="txtes" style="width:20em ;height:20em;"   maxlength="50000" required=""  readonly  value="<?php echo $estefa2 ; ?>"  
...
if ($StatusID==8 ) // تم رفع الملف و يجد استيفاء
{
  ...
  <tr>
        <td hidden >
		<div align="right">
         <input type="file" name="myfile_keta3" id="myfile_keta3" accept="application/pdf"   >
        </div>
		</td>

	
	
	<td class="text-center"><a  href="3ardfile.php?id=<?php echo $destination; ?>" target="_blank" >عرض</a>  </td>
        <td colspan="2">
 ...
 oninput="setCustomValidity('')"  pattern="^[\u0621-\u064A ]+$" title="حروف فقط غير مسموح باستخدام ارقام" />
        ...
        <td hidden >
		<div align="right">
         <input type="file" name="myfile_estefaa_keta3" id="myfile_estefaa_keta3" accept="application/pdf"   >
        </div>
		</td>
	...
	<td class="text-center">
		<a  href="3ardfile.php?id=<?php echo $keta3_estefaa_dest; ?>" target="_blank" >عرض</a>  </td>
        <td colspan="2">
		<div align="right"> ملف  الاستيفاء بعد رد لجنة القطاع
		</div>
		</td>
      </tr>
	  <tr>
	  <input type="hidden" name="destination" id="destination" value="<?php echo $keta3_estefaa_dest; ?>"  />	
	  </tr>
<?php }
...
else
{
   ...
   result2 = pattern.test(txtorg);
	...
	else
	{
		...
		{
			var strr="ملف اللائحه الداخليه يجب أن لا يزيد حجمه عن 5 ميجا"
	  alert(strr);
		}
		else
		{
	...
	{
		if( (fileSize2/1048576) > 5)
		{
			var strr="ملف الإمكانيات البشريه يجب ان لا يزيد حجمه عن 5 ميجا"
	  alert(strr);
		}
		else
		{
	...
	else
	{
		...
		{
			var strr="ملف اخرى يجب ان لا يزيد حجمه عن 5 ميجا"
	  alert(strr);
		}
		else
		{
