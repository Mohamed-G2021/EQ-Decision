...
//echo '--------------------------------------'.$n;
	...
	?>  
     <form class="form-horizontal" action="update_mo3adalat.php" method="post" enctype="multipart/form-data"  >
...
<!------------------------------------------------------------------------------------------------------------------------------->
 ...
 oninvalid="this.setCustomValidity('يجب إدخال اسم التخصص')"
...
<?php if($matlob_ID != 4){ ?>	
		...
		$sql = 'SELECT distinct `mo3adla_ID`,`mo3adla_VALUE` FROM `mo3adla` ';
		$stmt = $con->prepare($sql);
		//$stmt->bind_param('s', $FK_matlob);
		/* Execute statement */
		$stmt->execute();
		...
		while($row = $res->fetch_array(MYSQLI_ASSOC)) {
		   $mo3adla_ID =$row['mo3adla_ID'];
		   $mo3adla_VALUE=$row['mo3adla_VALUE'];

		  ?>
            
			<option value="<?php echo $mo3adla_ID ?>" ><?php echo $mo3adla_VALUE?></option>
...
?>
    ...
    <td>
		<div align="right">لجنة معادلات</div>
...
<tr>
        ...
        <td>
		<div align="right"> المذكره
		</div>
		</td>
		<input type="hidden" name="destination_mozakera" id="destination_mozakera" value="<?php echo"" ?>"  />
...
<?php } if($matlob_ID == 4){ ?>
   	<tr>
        <td >
		<div align="right">
         <input type="file" name="accept_dest" id="accept_dest" accept="application/pdf" required="" >
        </div>
		</td>
		
        <td>
		<div align="right"> ملف بموافقة لجنة القطاع
		</div>
		</td>
      </tr>
	  <tr>
	  <input type="hidden" name="accept_dest" id="accept_dest" value="<?php echo"" ?>"  />	
	  </tr>  
	  <?php } ?>
	  <!------------------------------------------------------------------------------------------------>
<?php }
...
<!------------------------------------------------------------------------------------------------------------>		  
 ...
 <input type="hidden" name="matlob_ID" value="<?php echo $matlob_ID;?>" />
        </div>
    </label>
	</td>
    </tr>
