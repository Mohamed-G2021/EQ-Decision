...
if (isset($_POST['btnsearch'])) {
    ...
    /*start to switch acknowledgement if request has acknowledgement*/
    if(isset($_POST['acknowledgement'])){

        if (isset($_POST['request_id']) && isset($_POST['acknowledgement'])) {
             $request_id = $_POST['request_id'];
             $acknowledgement = ($_POST['acknowledgement'] == "0") ? "1" : "0";
 
             // Update the acknowledgement in the database
             $sql = "UPDATE request SET acknowledgement = ? WHERE REQUEST_ID = ?";
             $stmt = $con->prepare($sql);
             $stmt->bind_param("ii", $acknowledgement, $request_id);
             $stmt->execute();
        }
     }
     /*end to switch acknowledgement if request has acknowledgement*/
...
/////////////////////////////////////////////////
    ...
    /*start get count of result to make pagination*/
     $count_sql = "SELECT COUNT(DISTINCT applicant.`Serial`) as total_results
        ...
        $total_results = $count_row['total_results'];
    ...
    if ($page > 1) {
        echo '<form method="post" action="' . htmlspecialchars($_SERVER["PHP_SELF"]) . '?page=' . ($page - 1) . '" class="paginationLink" >
        <input type="hidden" name="csrf_token" value="' . $_POST['csrf_token'] . '">
        <input type="hidden" name="request_id" value="' . $_POST['txtrequest'] . '">
        <input type="hidden" name="txtarname" value="' . $_POST['txtarname'] . '">
        <input type="hidden" name="txtrequest" value="' . $_POST['txtrequest'] . '">
        <input type="hidden" name="statusselect" value="' . $_POST['statusselect'] . '">
        <input type="submit" name="btnsearch" value="<<<"> 
        </form>';
    }

    if ($page > 1 || $total_pages > 1) {
        echo '<form method="post" action="' . htmlspecialchars($_SERVER["PHP_SELF"]) . '?page=1" class="paginationLink ' . ($page == 1 ? 'activePagination' : '') . '" style="display: inline-block; margin: 10px 5px;">
            <input type="hidden" name="csrf_token" value="' . $_POST['csrf_token'] . '">
            <input type="hidden" name="request_id" value="' . $_POST['txtrequest'] . '">
            <input type="hidden" name="txtarname" value="' . $_POST['txtarname'] . '">
            <input type="hidden" name="txtrequest" value="' . $_POST['txtrequest'] . '">
            <input type="hidden" name="statusselect" value="' . $_POST['statusselect'] . '">
            <input type="submit" name="btnsearch" value="1" style="background:transparent;border:0px">
        </form>';
    }    
    // Display the page numbers
    for ($i = $startPage; $i <= $endPage; $i++) {
        if ($i != 1 && $i != $total_pages) {
            $activeClass = ($page == $i) ? 'activePagination' : '';
            echo '<form method="post" action="' . htmlspecialchars($_SERVER["PHP_SELF"]) . '?page=' . $i . '" class="paginationLink ' . $activeClass . '" style="display: inline-block; margin: 10px 5px;">
                <input type="hidden" name="csrf_token" value="' . $_POST['csrf_token'] . '">
                <input type="hidden" name="request_id" value="' . $_POST['txtrequest'] . '">
                <input type="hidden" name="txtarname" value="' . $_POST['txtarname'] . '">
                <input type="hidden" name="txtrequest" value="' . $_POST['txtrequest'] . '">
                <input type="hidden" name="statusselect" value="' . $_POST['statusselect'] . '">
                <input type="submit" name="btnsearch" value="' . $i . '" style="background:transparent;border:0px"> 
            </form>';
        }
    }
    
    // Display the last page number
    echo '<form method="post" action="' . htmlspecialchars($_SERVER["PHP_SELF"]) . '?page=' . $total_pages . '" class="paginationLink ' . ($page == $total_pages ? 'activePagination' : '') . '" style="display: inline-block; margin: 10px 5px;">
        <input type="hidden" name="csrf_token" value="' . $_POST['csrf_token'] . '">
        <input type="hidden" name="request_id" value="' . $_POST['txtrequest'] . '">
        <input type="hidden" name="txtarname" value="' . $_POST['txtarname'] . '">
        <input type="hidden" name="txtrequest" value="' . $_POST['txtrequest'] . '">
        <input type="hidden" name="statusselect" value="' . $_POST['statusselect'] . '">
        <input type="submit" name="btnsearch" value="' . $total_pages . '" style="background:transparent;border:0px"> 
    </form>';
    ...
    echo "</div>";
    /*end get count of result to make pagination*/
  ...
  ,`request`.`keta3_estefaa_dest`,ss.datelog, request.acknowledgement
FROM applicant $fromm ,matlob,status,degree,status_log,request
...
<td class="text-center">';
                    ...
                    echo 
                        '<form method="post" action="' . htmlspecialchars($_SERVER["PHP_SELF"]) . '?page=' . $page . '" >
                            ...
                            <input type="hidden" name="acknowledgement" value="' . $row['acknowledgement'] . '">
                            <input type="hidden" name="txtarname" value="' . $_POST['txtarname'] . '">
                            <input type="hidden" name="txtrequest" value="' . $_POST['txtrequest'] . '">
                            <input type="hidden" name="statusselect" value="' . $_POST['statusselect'] . '">';

                            if ($row['acknowledgement'] == "0") {
                                echo ' <button type="submit" name="btnsearch" id="btnsearch" class="btn btn-success">تم الإطلاع';
                            } else {
                                echo ' <button type="submit" name="btnsearch" id="btnsearch" class="btn btn-danger">لم يتم الإطلاع';
                            }
...
<td class="text-center">' . $deg_name . ' </td>
<td class="text-center"> <a  href="show_log.php?reqid=' . $row['REQUEST_ID'] . '"> ' . $row['REQUEST_ID'] . '</a> </td>
<td class="text-center"> لم يتم التعديل بعد من قبل الجامعة ';
...
<td class="text-center">' . $deg_name . ' </td>
<td class="text-center"> <a  href="show_log.php?reqid=' . $row['REQUEST_ID'] . '"> ' . $row['REQUEST_ID'] . '</a> </td>
<td class="text-center"> لم يتم التعديل من قبل القطاع';
