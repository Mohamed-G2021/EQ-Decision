...
//////////////////////////////////////////////////////	
	...
	$q="
	SELECT applicant.`Serial`,applicant.`univ_id`,applicant.`USER_ID`,applicant.`degree`,applicant.`fk_degree`,applicant.`FK_matlob`,request.destination,`applicant`.`user_role` ,`request`.`karar`,`request`.REQUEST_ID,request.fk_tawseya
...
<!------------------------------------------------------------------------------------------------------------>
    <tr>
		...
		$sql = 'SELECT distinct tawseya_ID,tawseya_NAME FROM tawseya where tawseya_ID=? ';
		$stmt = $con->prepare($sql);
		$stmt->bind_param('s', $fk_tawseya);
		///* Execute statement */
		$stmt->execute();
		$res = $stmt->get_result();
