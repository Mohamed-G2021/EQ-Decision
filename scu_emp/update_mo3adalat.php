...
//echo '--------------------------------------'.$n;
	...
	$sql2 = "UPDATE `request` SET `StatusID` = 10 , `updated_date_mo3adalat`= ?,`USER_ID`=? ,`fk_mo3adla`=? ,`user_role`=? ,`mozakera_dest`=?,`accept_dest`=?  , `acknowledgement` = ? WHERE `FK_Applicant_serial` =?";
$stmt2 = $con->prepare($sql2);
$stmt2->bind_param("ssssssss",$date,$myUSER_ID,$fk_mo3adla,$roleid,$destinationn,$accept_destinationn,$acknowledgement,$serallll);
//echo"ggggg";
$stmt2->execute();
