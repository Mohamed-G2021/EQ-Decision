...
.pagination .w3-button.active {
    background-color: #4CAF50;
    color: white;
    border-radius: 5px;
}

.pagination .w3-button.active:hover {
    background-color: #45a049;
}

.pagination .w3-button:first-child {
    border-top-left-radius: 5px;
    border-bottom-left-radius: 5px;
}

.pagination .w3-button:last-child {
    border-top-right-radius: 5px;
    border-bottom-right-radius: 5px;
}

.pagination .w3-button:not(:last-child) {
    margin-right: 2px;
}
</style>
...
if (isset($_POST['btnsearch'])) {
    ...
    $_SESSION['request'] = stripslashes($_POST['txtrequest']);
    }
    if (isset($_POST['statusselect'])) {
    $_SESSION['statusselect'] = $_POST['statusselect'];
    }
    if (isset($_POST['typeselect'])) {
    $_SESSION['typeselect'] = $_POST['typeselect'];
    }

    $request = $_SESSION['request'];
    $statusselect = $_SESSION['statusselect'];
    $typeselect = $_SESSION['typeselect'];*/
...
//**************************************** ولا رقم طلب ولا حاله **********************************************************************//
    ...
    if ($typeselect != 0) {
        $wheree = "and `applicant`.`user_role` = $typeselect";
    }
...
echo"<br>";
 */
    /*$sql2 = "SELECT count(*) as total_results
    ...
    $sql = "SELECT applicant.`Serial` as app_serial ,`applicant`.`univ_id` as u_i_a,`applicant`.`user_role` as u_role $selectt ,`degree`,`FK_matlob`,matlob.matlob_NAME,request.StatusID,request.REQUEST_ID,
  status.STATUS_VALUE,status.STATUS_ID,`request`.`destination`,`request`.`univ_destination`,`request`.`univ_emkanyat`,`request`.`univ_gadwa`,`request`.`scu_destination`,`request`.`mozakera_dest`,
  `request`.`estefaa_dest` ,`request`.`estefa2`,`request`.`fk_session_id` ,`request`.`accept_dest`
...
and request.FK_Applicant_serial = `applicant`.`Serial`
 ...
 $wheree /*LIMIT $start_result, $results_per_page*/
...
and request.fk_session_id=$fk_session_id";
//echo $sessionpathq;
...
//$stmtsession->bind_param("s", $fk_session_id);
                ...
                while ($rowsession = $ressession->fetch_array(MYSQLI_ASSOC)) {$session_path = $rowsession['path'];
	 ...
	 <td class="text-center"><a  href="3ardfile.php?id=' . $session_path . '" target="_blank" >عرض</a>  </td>';
	  ...
	  <td class="text-center"> </td>
	   <td class="text-center"> ' . $id_session . '  </td>
...
<td class="text-center">' . $row['REQUEST_ID'] . ' </td>
	 ...
	 <td class="text-center"><a  href="3ardfile.php?id=' . $session_path . '" target="_blank" >عرض</a>  </td>';
			  ...
			  <td class="text-center">' . $row['estefa2'] . '</td>';
                        ...
                        echo ' <td class="text-center"><a  href="3ardfile.php?id=' . $row['univ_gadwa'] . '" target="_blank" >عرض</a>  </td>
